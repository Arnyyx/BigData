...
top_10_gold_medal_countries_2012 = vdv_olympics_df.filter((col('Year') == 2012) & (col('Season') == 'Summer') & (col('Medal') == 'Gold'))\
.groupBy('NOC').agg(count('*').alias('gold_medals_count')).orderBy('gold_medals_count', ascending=False).limit(10)
