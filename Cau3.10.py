...
_10_athletes_20th_century = vdv_olympics_df.filter((col('Year') >= 1900) & (col('Year') <= 2000) & (col('Medal') == 'Gold')).groupBy('Name').agg(count('*').alias('gold_medals_count')).orderBy('gold_medals_count', ascending=False).limit(10)
